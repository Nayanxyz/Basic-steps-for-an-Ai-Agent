...
if user_input := st.chat_input("Ask about the company or weather..."):
    ...
    if len(st.session_state.chat_history) > 6:
        ...
        # 1. Get the summary (BUT HIDE THE NEWEST QUESTION!)
        history_to_compress = st.session_state.chat_history[:-1]
        compressed_text = compress_memory(history_to_compress)

        # --- DEBUGGER: Let's see what the AI actually remembered ---
        print(f"SECRET SUMMARY:\n{compressed_text}\n")
        ...
        # 3. Create the Meat
        summary_meat = {"role": "system", "content": f"Previous Conversation Fact Sheet:\n{compressed_text}"}
